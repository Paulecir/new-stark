...
ALTER TABLE `balance` DROP PRIMARY KEY,
    MODIFY `wallet` ENUM('MAIN', 'BINARY_LEFT_POINT', 'BINARY_RIGHT_POINT') NOT NULL DEFAULT 'MAIN',
    ADD PRIMARY KEY (`user_id`, `wallet`);

-- AlterTable
ALTER TABLE `balance_history` MODIFY `wallet` ENUM('MAIN', 'BINARY_LEFT_POINT', 'BINARY_RIGHT_POINT') NOT NULL DEFAULT 'MAIN';
