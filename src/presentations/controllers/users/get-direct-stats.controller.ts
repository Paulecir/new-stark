...
export const getDirectStatsController = async (requestData: IRequest) => {
    try {
        ...
        const user = await DirectService.getDirectStatsById({ id: requestData.query.parentId || requestData.user.id, user: requestData.user, level: requestData.query.level });
