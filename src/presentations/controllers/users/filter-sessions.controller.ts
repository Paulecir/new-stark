 ...
 */
export const filterUserSessionsController = async (requestData: IRequest) => {
    try {
        // Recupera o usuário pelo ID usando o UserService
        const userFilter = await UserService.filterSessions([], { page: requestData.pagination.page || 1, pageSize: requestData.pagination.pageSize || 1 }, { created_at: "desc" }, requestData.user);
